...
p=0.8 % Energy's Probabilistic parameter 
...
for i=1:n
    if(min_E ==max_E)
        Q(i) = ( NET(i).hop *(hop_C/100 * abs(log(1/hop_C^2))));
        NET(i).Q = Q(i);
    else
        %Q(i) = (p*(NET(i).E - min_E)/(max_E-min_E)+(q1/NET(i).hop));
        Q(i) = EnergyEXTReward(NET(i).E, min_E, NET(i).hop, max_E);
        NET(i).Q = Q(i);
...
for i=1:n
    ...
    display(strcat( ' Here is Executing: ', num2str(i)));
end
end
...
while(op_nodes>0 && rnd<5000)
            ...
            if cluster(i,j).dest == CH(i).id
                if cluster(i,j).E>0
                    ...
                    if (cluster(i,j).dts_ch<=range_C | cluster(i,j).Q ==maxQ) 
                        ...
                        R = EnergyEXTReward(cluster(i,j).E, min_E, cluster(i,j).hop, max_E);
    ...
    for j =1:CH_tot
        ...
        if CH(j).E >thres && thres>0
            ...
            R = EnergyEXTReward(CH(j).E, min_E, CH(j).hop, max_E); 
    ...
    for l=1:n
        if NET(l).E>0
            if NET(l).dts<=range_C && NET(l).role==0
                ...
                R = EnergyEXTReward(NET(l).E, min_E, NET(l).hop, max_E); 
